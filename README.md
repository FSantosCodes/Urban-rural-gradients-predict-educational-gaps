...
To organize this report, here is described where to get input and output files, as well as the folders used in this repository to get the R scripts and the shiny app source code: 
...
The algorithm processing is demanding and users should be proceed with caution since large computations can fail (we experience a recursive interruption after ~2500 iterations without an explanation but it is suspicious a RAM overflow or a CPU temperature breakpoint). Therefore, it is important to monitor processed .csv files to advice the last processing point and restart processing. If users require an update or help using this routine, you can email  **fabian_santos_@hotmail.com** to discuss a collaboration.
